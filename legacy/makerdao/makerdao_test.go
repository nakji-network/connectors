...
func TestMain(*testing.M) {
	mockConnector = &Connector{
		...
		topicPrefix: ".fct.nakji.makerdao.",
...
func TestParse(t *testing.T) {
	...
	for _, testCase := range []struct {
	...
	}{
		{
			...
			want: &kafkautils.Message{
				Key:   kafkautils.NewKey("makerdao_ethereum", "0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2"),
				Topic: kafkautils.MustParseTopic(".fct.nakji.makerdao.0_0_0.eth_withdrawal", mockConnector.Env),
				ProtoMsg: &ETH.Withdrawal{
		...
		},
		{
			...
			want: &kafkautils.Message{
				Key:   kafkautils.NewKey("makerdao_ethereum", "0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2"),
				Topic: kafkautils.MustParseTopic(".fct.nakji.makerdao.0_0_0.eth_deposit", mockConnector.Env),
				ProtoMsg: &ETH.Deposit{
					Ts: &timestamppb.Timestamp{
						Seconds: 1630580307,
					},
		...
		},
		{
			...
			want: &kafkautils.Message{
				Key:   kafkautils.NewKey("makerdao_ethereum", "0x6B175474E89094C44Da98b954EedeAC495271d0F"),
				Topic: kafkautils.MustParseTopic(".fct.nakji.makerdao.0_0_0.mcd_dai_transfer", mockConnector.Env),
				ProtoMsg: &MCD_DAI.Transfer{
					Ts: &timestamppb.Timestamp{
						Seconds: 1630580307,
					},
...
func registerTopics(version string) {
	topicTypes := map[string]proto.Message{
		fmt.Sprintf("nakji.makerdao.%s.uni_transfer", version):  &UNI.Transfer{},
		fmt.Sprintf("nakji.makerdao.%s.eth_deposit", version):   &ETH.Deposit{},
		fmt.Sprintf("nakji.makerdao.%s.usdt_transfer", version): &USDT.Transfer{},
	}
